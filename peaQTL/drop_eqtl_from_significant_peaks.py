...
Writes filtered BED files, summaries, and plots under:

    FILTERED_SIG_PEAKS_DIR

Soft mode
---------
When ``--soft`` is supplied, reads:
...
1. Removes complete significant peaks that overlap at least one exact
   significant GTEx liver eQTL position.
2. Writes:
       <cell_type>_significant_peaks_without_eqtl.bed.gz
3. Creates a pie chart showing overlapping versus nonoverlapping peaks.
4. Creates two global bar plots across all cell types.
5. Writes a CSV filtering summary.
...
SUMMARY_FILENAME: Final[str] = (
    "significant_peaks_eqtl_filtering_summary.csv"
)

PLOTS_DIRECTORY_NAME: Final[str] = "eqtl_overlap_pie_charts"
...
def resolve_mode_paths(
    ...
    """Return the input, output, summary, and plot paths for the run mode.

    In default mode, the existing constants are used exactly as before.

    In soft mode, the summary and plot directories are derived from
    ``SOFT_FILTERED_SIG_PEAKS_DIR`` so that every soft-mode output remains
    isolated from the default outputs.
    """

    if soft:
        input_dir = SOFT_UNFILTERED_SIG_PEAKS_DIR
        output_dir = SOFT_FILTERED_SIG_PEAKS_DIR
        summary_path = output_dir / SUMMARY_FILENAME
        plots_dir = output_dir / PLOTS_DIRECTORY_NAME
    else:
        input_dir = UNFILTERED_SIG_PEAKS_DIR
    ...
    return input_dir, output_dir, summary_path, plots_dir
...
def parse_gtex_variant_ids(
    variant_ids: pd.Series,
) -> pd.DataFrame:
    """Parse GTEx variant IDs into unique one-base GRCh38 intervals."""

    ids = variant_ids.astype("string").str.strip()

    if ids.isna().any() or (ids == "").any():
        raise ValueError(
            "The GTEx eQTL table contains empty variant IDs."
        )
    ...
    unexpected_builds = sorted(
        set(
            parsed.loc[
                parsed["build"] != EXPECTED_GTEX_BUILD,
                "build",
            ].dropna()
        )
    )
    ...
    if unexpected_builds:
        raise ValueError(
            ...
            f"Expected only {EXPECTED_GTEX_BUILD!r}; "
            f"found {unexpected_builds}."
        )
    ...
    intervals = pd.DataFrame(
        {
            BIOFRAME_CHROM_COLUMN: (
                parsed["chrom_raw"].map(normalize_chromosome)
            ),
...
def load_eqtl_intervals(
    ...
    if not eqtl_path.is_file():
        raise FileNotFoundError(
            f"GTEx liver eQTL file not found: {eqtl_path}"
        )

    LOGGER.info(
        "Reading GTEx liver eQTLs from %s",
        eqtl_path,
    )
    ...
    n_unique_variant_ids = int(
        eqtl_table[VARIANT_ID_COLUMN].nunique()
    )

    unique_variant_ids = (
        eqtl_table[VARIANT_ID_COLUMN].drop_duplicates()
    )

    eqtl_intervals = parse_gtex_variant_ids(
        unique_variant_ids
    )
...
def read_bed_preserving_values(
    bed_path: Path,
) -> pd.DataFrame:
    ...
    if bed.shape[1] < BED_MIN_COLUMNS:
        raise ValueError(
            f"{bed_path} has {bed.shape[1]} columns; "
            f"expected at least {BED_MIN_COLUMNS} BED columns."
        )
    ...
    if invalid_coordinate_mask.any():
        ...
        raise ValueError(
            f"{bed_path} contains invalid BED coordinates at "
            f"zero-based row indices {invalid_rows}."
        )

    return bed


def build_bioframe_peak_table(
    peaks: pd.DataFrame,
) -> pd.DataFrame:
...
def make_temporary_plot_path(
    output_path: Path,
) -> Path:
    """Create a temporary path that retains the real image suffix."""
...
def save_figure_atomic(
    ...
    temporary_path = make_temporary_plot_path(
        output_path
    )
...
def create_pie_chart(
    ...
    *,
    ...
    """Create an eQTL-overlap pie chart for one cell type."""
    ...
    plt.title(
        f"{cell_type}: significant peaks vs. liver eQTL overlap"
    )
...
def create_total_counts_bar_plot(
    ...
    *,
    ...
    """Create a total-count bar plot for cell types and eQTLs."""
    ...
    ax.set_title(
        "Total significant-peak counts by cell type "
        "and liver eQTL set size"
    )
...
def create_total_counts_with_overlap_lines_plot(
    ...
    *,
    ...
    """Create total-count bars with eQTL-overlap lines."""
    ...
    ax.set_title(
        "Total significant-peak counts with "
        "eQTL-overlapping peak counts per cell type"
    )
    ...
    maximum_height = max(total_heights)
    vertical_offset = max(
        maximum_height * 0.01,
        1.0,
    )

    for index, overlap_count in enumerate(
        overlap_counts
    ):
        bar = bars[index]
        ...
        left_x = bar.get_x()
        right_x = (
            left_x + bar.get_width()
        )

        label = (
            "eQTL-overlapping peaks"
            if not added_legend_label
            else None
        )
...
def discover_input_files(
    ...
    if not input_dir.is_dir():
        raise FileNotFoundError(
            "Unfiltered significant-peaks directory "
    ...
    if empty_cell_types:
        raise ValueError(
            "Could not infer cell types from filenames: "
            f"{empty_cell_types}"
        )

    if len(cell_types) != len(set(cell_types)):
        raise ValueError(
            "Multiple input files resolve to the "
            "same cell-type name."
        )
...
def make_output_path(
    ...
    """Infer cell type and filtered output path."""
...
def make_pie_chart_path(
    ...
    return (
        plots_dir
        / f"{cell_type}{PIE_CHART_SUFFIX}"
    )


def get_global_plot_paths(
    plots_dir: Path,
) -> tuple[Path, Path]:
    """Return the two global plot paths."""

    return (
        plots_dir
        / TOTAL_COUNTS_BAR_PLOT_FILENAME,
        plots_dir
        / TOTAL_COUNTS_WITH_OVERLAP_FILENAME,
    )
...
def validate_output_paths(
    ...
    """Refuse replacement unless ``--overwrite`` is supplied."""
    ...
    for input_path in input_paths:
        cell_type, output_path = make_output_path(
            input_path,
            output_dir,
        )

        pie_chart_path = make_pie_chart_path(
            cell_type,
            plots_dir,
        )
...
def process_peak_file(
    ...
    original_peaks = read_bed_preserving_values(
        input_path
    )
    ...
    if original_peaks.empty:
        ...
        peak_intervals = build_bioframe_peak_table(
            original_peaks
        )
    ...
    if n_input != n_removed + n_output:
        raise RuntimeError(
            "Peak-count integrity check failed for "
            f"{input_path}."
        )
...
def run(
    ...
    """Run exact-eQTL filtering in default or soft mode."""
    ...
    if total_input != (
        total_removed + total_output
    ):
        raise RuntimeError(
            "Dataset-wide peak-count integrity "
            "check failed."
        )
    ...
    LOGGER.info(
        "Completed %d cell types in %s mode: "
        "retained %d/%d peaks; removed %d "
        "(%.2f%%). Summary: %s | "
        "Global plots: %s ; %s",
        len(summary),
        mode_name,
        ...
        total_removed,
        (
            100.0 * total_removed / total_input
            if total_input
            else 0.0
        ),
        summary_path,
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Remove significant peaks that overlap "
            "exact GTEx liver eQTL positions and "
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Replace existing filtered BED files, "
            "plots, and summary."
        ),
    )
...
def main() -> None:
    ...
    try:
        ...
        LOGGER.exception(
            "Significant-peak eQTL filtering failed"
        )
        raise
